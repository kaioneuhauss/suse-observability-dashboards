...
Os dois Jobs são hooks `post-install,post-upgrade`, têm prazo máximo, recursos limitados e Service Tokens configuráveis. O laboratório usa identidades separadas; o perfil simples de cliente novo permite um único Secret aprovado. Eles terminam após publicar. Não precisam montar o token Kubernetes do ServiceAccount. Antes de substituir um dashboard, verificam propriedade/identidade e limitam a operação aos objetos gerenciados. A recriação pode alterar IDs; reabra pela lista, não por um bookmark antigo.
